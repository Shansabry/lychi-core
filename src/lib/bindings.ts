...
async agentChatStart(system: string, user: string, fresh: boolean, withTools: boolean, generation: number, images: string[], display: MessageDisplay | null) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("agent_chat_start", { system, user, fresh, withTools, generation, images, display }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
async getAgentPlan(input: string) : Promise<Result<AgentPlan | null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("get_agent_plan", { input }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
 ...
 */
async storeAgentPlan(plan: AgentPlan) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("store_agent_plan", { plan }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
},
async executeAgentPlan(planId: string) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("execute_agent_plan", { planId }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
 ...
 */
export type AgentPlan = { id: string; input: string; steps: AgentStep[] }
/**
 * A single step in an agent plan.
 */
export type AgentStep = { action_id: string; args: string; label: string; risk: RiskLevel }
